...
// note:
        function parseVisibleItemsSingleData(chart, handler) {
            var datasets = chart.data.datasets;
            var meta, i, j, ilen, jlen;
...
module.exports = {
	...
	modes: {
        ...
        // note:
        mx: function(chart, e, options) {
            var position = getRelativePosition(e, chart);
            var items = [];
            var intersectsItem = false;

            var get_data = 0;
            parseVisibleItemsSingleData(chart, function(element) {
                // note:
                // if (element.inXRange(position.x)) { // original
                if (element.useXRange(position.x)) {
                    items.push(element);
                    get_data = 1;
                }
...
// note:
        function museXRange(mouseX) {
            var vm = this._view;
            // return vm ? (Math.abs(mouseX - vm.x) < 1.5 + 1) : false;
			return vm ? (Math.abs(mouseX - vm.x) < vm.radius + vm.hitRadius) : false;
        }
