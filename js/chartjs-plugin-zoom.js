...
function zoomTimeScale(scale, zoom, center, zoomOptions) {
	...
	// note when chart.js format is time - change zoom label
	var time_end = options.time.min;
	localStorage.setItem("js_value_zoom_time_label_min",  options.time.min.format('YYYY/MM/DD HH:mm:ss') );
	localStorage.setItem("js_value_zoom_time_label_max",  options.time.max.format('YYYY/MM/DD HH:mm:ss') );

	localStorage.setItem("js_value_zoom_time_label",  "now Label " + options.time.min.format('YYYY/MM/DD HH:mm:ss') + " ~ " +
		options.time.max.format('YYYY/MM/DD HH:mm:ss') + " length " + time_end.fromNow() );

	// note : set custom Event (zoom)
	onChanged( options.time.min.format('YYYY/MM/DD HH:mm:ss'),options.time.max.format('YYYY/MM/DD HH:mm:ss'));
}
...
// note : custom Event Listener
function onChanged(start , end) {
	var evt = $.Event('mChanged');
	evt.mStart = start;
	evt.mEnd = end;
	$(window).trigger(evt);
}

function panTimeScale(scale, delta, panOptions) { // note when time(y) scroll
	var options = scale.options;
	...
	// note
	var min_date = new Date((options.time.min)); // delete *1000 from original - date number to date
	var max_date = new Date((options.time.max)); // date number to date
	localStorage.setItem("test_value",  "now Label " + moment(min_date).format('YYYY/MM/DD HH:mm:ss') + " ~ " +  moment(max_date).format('YYYY/MM/DD HH:mm:ss'));

	localStorage.setItem("js_value_zoom_time_label_min", moment(min_date).format('YYYY/MM/DD HH:mm:ss') );
	localStorage.setItem("js_value_zoom_time_label_max", moment(max_date).format('YYYY/MM/DD HH:mm:ss') );

	// note : set custom Event
	onChanged(moment(min_date).format('YYYY/MM/DD HH:mm:ss'),moment(max_date).format('YYYY/MM/DD HH:mm:ss'));
}
...
var zoomPlugin = {
	...
	afterInit: function(chartInstance) {
		...
		chartInstance.resetZoom = function() {
			...
			//	dataset._meta = null;  // fix when reset ...chart always fresh
			});
